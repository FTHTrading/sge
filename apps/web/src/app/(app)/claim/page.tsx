  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto">
      ...
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-8">
        {/* Info banner */}
        <div className="flex items-start gap-3 rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-4 mb-8">
          ...
          <div className="text-sm text-white/60">
            ...
            <span className="text-white font-medium">{CLAIM_AMOUNT_HUMAN} {selectedToken}</span>{" "}
            transfer, then receive{" "}
            <span className="text-white font-medium">{SGE_PAYOUT.toLocaleString()} SGE</span>{" "}
            tokens.
          </div>
        </div>
            ...
            {(Object.keys(TOKENS) as TokenSymbol[]).map((symbol) => (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-xs text-white/30">{TOKENS[symbol].symbol}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between rounded-lg border border-white/[0.06] bg-white/[0.02] p-4 mb-6">
          <div className="text-center">
            ...
            <div className="text-lg font-bold text-white">{CLAIM_AMOUNT_HUMAN} {selectedToken}</div>
          </div>
          <ArrowRight className="w-5 h-5 text-emerald-400" />
          <div className="text-center">
            <div className="text-xs text-white/30 uppercase tracking-wider mb-1">You Receive</div>
            <div className="text-lg font-bold text-emerald-400">{SGE_PAYOUT.toLocaleString()} SGE</div>
          </div>
        </div>
          ...
          >
            {isProcessing ? (
              ...
              <span className="flex items-center justify-center gap-2">
                ...
                Claim {SGE_PAYOUT.toLocaleString()} SGE for {CLAIM_AMOUNT_HUMAN} {selectedToken}
              </span>
            )}
          </button>
        )}
